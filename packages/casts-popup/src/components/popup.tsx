...
const _Popup = forwardRef((props: PopupProps, ref: Ref<PopupRef>) => {
  ...
  // NOTE: popuo content only can be mounted after trigger element had been mounted
  const [childrenElementMounted, setChildrenElementMounted] = useState(false);

  useEffect(() => {
    setChildrenElementMounted(true);
  }, []);

  const renderPopupContent = () => {
    if (disabled || !childrenElementMounted) {
      return null;
    }
