...
export const useThemeGenerator = (props: UseThemeGeneratorProps) => {
  ...
  useEffect(() => {
    ...
    // component mounted should disabled transition,
    // debounceGeneratePalettes is a setTimeout function,
    // so, we can execute the following code after debounceGeneratePalettes
    const noTransitionStyleCode = '* { transition: none !important; }';

    const style = document.createElement('style');
    style.innerHTML = noTransitionStyleCode;
