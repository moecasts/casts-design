...
export const useRangeInput = (props: UseRangeInputProps) => {
  ...
  const handleBlur: FocusEventHandler<HTMLInputElement> = (e) => {
    // Delay check to handle focus change cases
    setTimeout(() => {
      // Only trigger blur if active element is not within either input
      if (
  ...
  const handleKeyDown = (
    index: typeof START_INDEX | typeof END_INDEX,
    ...
    if (e.key === 'Tab') {
      ...
      const fns = {
        [START_INDEX]: () => endInputRef.current?.focus(),
        [END_INDEX]: () => startInputRef.current?.focus(),
      };
